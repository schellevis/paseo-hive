...
6. **FOLLOW-UP ROUND.** Reuse each seat's agent through `send_agent_prompt`; never start a new agent for a later round. Give every seat the anonymised ledger, the paths of the other seats' output, any user items, and its targets (goals.md). No follow-up prompt goes out until `hive.py gate` prints `ok` for the previous round. Repeat rounds up to the cap (user's choice, 1–10).
7. **Ending a round.** A round ends when every seat has delivered its turn. A seat that errors or shows no new activity for 10 minutes is skipped for that round; record the skip in the Moderator part of the ledger as `Skipped: <letter> (<reason>)` and in the checkpoint. A seat skipped twice in one session is replaced (catch-up prompt) or dropped, and the user is told which.
...
10. **CHECKS.** Run the goal's minimum-engagement check plus three universal checks (goals.md): every crux is resolved or has a "what would settle it"; every user item was addressed by at least one seat; every load-bearing assumption was checked or is weighed as one. If a check fails and the cap allows another round, run one targeted extra round; otherwise mark the checkpoint `Debate incomplete: <which check failed>`. In `--deep`, run the fairness check (prompts.md) on the checkpoint draft and fix flagged misrepresentations.
...
- **Before the next round's prompts go out, and before the checkpoint:** `hive.py gate --session <dir> --leg <n> --round <n|select> [--skip <letters>]` checks that every active seat's answer was saved by `ingest` and not changed since (or that the seat is recorded as skipped), that the Items part of the ledger was built by `ledger` from the current answers and not edited, that a follow-up round has its `Round <n>` entry in `changes.md`, that no ID dangles, and that `leaks` is clean. `ingest` and `ledger` record what they wrote in `leg-<n>/records.json`; never edit it. Fix everything it prints; it exits 0 only when the round is complete.
...
leg-1/records.json       what hive.py ingest and ledger wrote, checked by hive.py gate; never edit
